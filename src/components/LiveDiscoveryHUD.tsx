...
export function LiveDiscoveryHUD({
  ...
  useEffect(() => {
    // If external logs provided (e.g. from CommandCenter stream), use those and don't open stream
    ...
    <div className="flex h-full min-w-0 flex-col overflow-hidden rounded-lg border border-zinc-800 bg-zinc-950/80 font-mono text-sm shadow-xl">
      <div className="flex items-center justify-between border-b border-zinc-800 px-4 py-2">
        <div className="flex items-center gap-2">
          <div className={`h-2 w-2 shrink-0 rounded-full ${error ? 'bg-red-500' : 'bg-emerald-500'}`} />
        ...
        <span className="text-xs text-zinc-500" title="Logs are streamed from the backend (GET /api/discovery/stream). Same text every time = backend sending fixed sequence.">From API</span>
      </div>
      <div
        ref={containerRef}
